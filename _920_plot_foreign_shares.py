...
_whitelist = pd.read_csv(COUNTRY_WHITELIST)["country"].unique()
...
def make_foreign_multiaff_plot(fname, lhs, rhs, figsize, cmap,
    ...
    for idx, ax in enumerate(axes):
        ...
        ax[0].text(val+3, -0.5, f"{val:g}%", fontsize=8)
        ...
        if idx == 0:
            ax[0].title.set_text('Share of multiple aff. authors with co-affiliation abroad')
...
def main():
    ...
    dtypes = {"author": "uint64", "multiaff": "float16", "year": "uint16"}
    ...
    # Compute share of authors with MA by year
    overall = shares.groupby(["year"])[["size", "sum"]].sum()
    print(">>> Shares of unique MA authors by year")
    print((overall["sum"]/overall["size"] * 100).round(2))
    ...
    for year in (1996, 2016):
        ...
        cor = lhs[["Foreign", "share"]].corr().iloc[1, 0]
        print("... Correlation MA intensity & share foreign "\
              f"aff authors: {cor:.3f}")
        # ...all countries
        fname = f"{OUTPUT_FOLDER}Figures/foreignshare-partner_country-all_"\
                f"{year}-{year+LENGTH-1}.pdf"
