...
def custom_formatter(x, na_rep=f"<{THRESHOLD_MA}"):
...
def make_stacked_area_plot(df, fname):
    ...
    plt.margins(0, 0)
    fig.savefig(fname, bbox_inches="tight", width=20)
    plt.close(fig)
...
def sort_df(df, year=2019, ascending=False):
...
def main():
    # Read in
    dtypes = {"year": "uint16", "multiaff": "float32"}
    df = read_source_files(["year", "inst_types", "multiaff", "author"],
                           dtype=dtypes)
    df["multiaff"] = df["multiaff"].fillna(0).astype("uint32")
    df = (df.sort_values("multiaff", ascending=False)
            .drop_duplicates(subset=["author", "field", "year"])
            .rename(columns={"year": "Year"})
            .dropna(subset=["inst_types"]))
    df["inst_types"] = df["inst_types"].apply(clean_inst_types)

    # Temporary correction
    mask = (~df["inst_types"].str.contains("-")) & (df["multiaff"] == 1)
    df.loc[mask, "multiaff"] = 0
    ...
    df_comb = grouping_with_shares(df, "Year", 'inst_types', aggs)
    del df
    ...
    # Stacked area plots for affiliation type combinations
    ...
    for field in df_ma["field"].unique():
        subset = prepare_subset(df_ma[df_ma["field"] == field])
        fname = "{}Figures/combs_share-fields-{}.pdf".format(
                OUTPUT_FOLDER, field)
    ...
    order = list(df_ma.T.sort_values(2019, ascending=False).index)
    order.remove("Other")
    order.append("Other")
    df_ma.columns.name = ""
